...
async def on_message(message):
	...
	if message.content.find('!help') != -1:
		await message.channel.send(
			...
			'!Red, !WR\n-!WTF1, !Fdank, !Fia, !F1```')
	# F1 twitter commands
	[await message.channel.send(f1twit_dict[command]) for command in f1twit_dict
	 if message.content.find(command) != -1]
	# Addition commands
	[await message.channel.send(addition_commands[command]) for command in addition_commands
	 if message.content.find(command) != -1]
