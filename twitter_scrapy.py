...
def twitter_scrape():
    ...
    for username in twitter_handles:
        tweetCriteria = got.manager.TweetCriteria().setUsername(
            username).setMaxTweets(count).setEmoji('unicode')
        tweets = got.manager.TweetManager.getTweets(tweetCriteria)
        # Grab specified fields for each tweet
        user_tweets = [[tweet.username, tweet.date,
                        tweet.text, tweet.permalink] for tweet in tweets]
        ...
        for tweet in user_tweets:
            ...
            post = {"handle": handle, "time": time,
                    "text": text, "link": link, "posted": False}
...
# Looks through our information stored in our mongodb and returns the links of the documents 
...
def twitter_filter():
    ...
    for word in twitter_words:
        ...
        for result in results:
            # Check if we've posted before, if not append the link to our list and set the posted 
            # value to true in the document.
            if result and not result["posted"]:
                link_list.append(result['link'])
                db.tweets.update_one({"_id": result["_id"]}, {
                                     "$set": {"posted": True}})
            else:
                pass
    return(link_list)
